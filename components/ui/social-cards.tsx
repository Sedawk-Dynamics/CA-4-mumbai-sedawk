...
export default function SocialCards({ cards, cardWidth = 260, cardHeight = 340, containerHeight = 420 }: SocialCardsProps) {
  ...
  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem]" style={{ height: `${containerHeight}px` }}>
          {cards.map((card, index) => {
            ...
            if (card.linkUrl && card.linkUrl.startsWith("http")) {
              return (
                <a key={index} href={card.linkUrl} target="_blank" rel="noopener noreferrer"
                  className="fan-card block cursor-pointer absolute"
            ...
            if (card.linkUrl) {
              return (
                <Link key={index} href={card.linkUrl}
                  className="fan-card block cursor-pointer absolute"
                  style={{ width: `${cardWidth}px`, height: `${cardHeight}px` }}>
                  {inner}
                </Link>
            ...
            return (
              <div key={index} className="fan-card absolute" style={{ width: `${cardWidth}px`, height: `${cardHeight}px` }}>
                {inner}
              </div>
            );
          })}
      ...
      {needsPagination && (
        <div className="flex items-center justify-center gap-4 mt-4 md:mt-6 z-30">
          ...
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <span key={i} className={`w-2 h-2 rounded-full transition-all duration-300 ${i === centerIndex ? "bg-[var(--navy)] scale-[1.3]" : "bg-black/15"}`} />
            ))}
          </div>
